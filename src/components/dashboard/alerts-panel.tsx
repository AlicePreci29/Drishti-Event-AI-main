              ...
              alerts.map((alert) => {
                ...
                return (
                  <AlertUI key={alert.id} variant={alert.variant} className="flex items-start gap-4">
                    <Icon className={cn("h-5 w-5 mt-1 shrink-0", alert.variant === "destructive" ? "text-destructive-foreground" : "text-primary")} />
                    <div className="grid gap-1 flex-grow">
                      <p className={cn("font-semibold", alert.variant === "destructive" && "text-destructive-foreground")}>
                        {alert.title}
                      </p>
                      <p className={cn("text-sm", alert.variant === "destructive" ? "text-destructive-foreground/80" : "text-muted-foreground")}>
                        {alert.description}
                      </p>
                       <p className={cn("text-xs", alert.variant === "destructive" ? "text-destructive-foreground/60" : "text-muted-foreground/80")}>
                        {alert.time}
                      </p>
                      {alert.action && (
                        <Button 
